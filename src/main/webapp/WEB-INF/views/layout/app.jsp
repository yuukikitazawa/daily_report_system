...
<body>
    <div id="wrapper">
        <div id="header">
            <div id="header_menu">
                <h1><a href="<c:url value='/?action=${actTop}&command=${commIdx}' />">日報管理システム</a></h1>&nbsp;&nbsp;&nbsp;
                    ...
                    <a href="<c:url value='?action=${actRep}&command=${commIdx}' />">フォロー管理</a>&nbsp;
                </c:if>
                    <a href="<c:url value='?action=${actRep}&command=${commIdx}' />">フォロー中ユーザーの日報</a>&nbsp;
                </c:if>
            </div>
            <c:if test="${sessionScope.login_employee != null}">
        ...
        <div id="footer">by Yuuki Kitazawa.</div>
    </div>
</body>
</html>
